...
int send_message(t_data *data, char *str, long mtype)
{
	...
	tmp.team_number = data->team_number;
...
t_msg_data *get_next_message_team(t_data *data, long mtpye)
{
	...
	if (!tmp)
		return (NULL);
	else if (tmp->team_number == data->team_number)
		return (tmp);
	msg_return.team_number = tmp->team_number;
